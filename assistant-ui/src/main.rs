...
#[tauri::command]
async fn run_doctor(app: tauri::AppHandle, args: Vec<String>) -> Result<(), String> {
    let binary_path = "/Users/aryankumawat/Friday/target/debug/assistant-cli";
    let mut cmd = tokio::process::Command::new(binary_path);
    let mut full_args = vec!["doctor".to_string()];
    full_args.extend(args);
    cmd.args(full_args);
    cmd.stdout(std::process::Stdio::piped());
    cmd.stderr(std::process::Stdio::piped());
    ...
    tokio::spawn(async move {
        while let Ok(Some(line)) = lines.next_line().await {
            let _ = app_clone.emit("assistant:doctor", line);
        }
    });
    Ok(())
}
...
fn main() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![start_assistant, run_doctor])
